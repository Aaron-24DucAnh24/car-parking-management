...
class Service {
  ...
  public async getStatistic(type: string) {
    let objList = this.database.get("data");
    ...
    for (let obj of objList) {
      if (condition(obj.inTime)) {
        ...
        for (let option of obj.options) {
          if (option.name === "washing") washingIn += 1;
          if (option.name === "oilChanging") oilChangingIn += 1;
          if (option.name === "wheelChecking") wheelCheckingIn += 1;
        }
      }
      if (obj.outTime && condition(obj.outTime)) {
        ...
        for (let option of obj.options) {
          if (option.name === "washing") washingOut += 1;
          if (option.name === "oilChanging") oilChangingOut += 1;
          if (option.name === "wheelChecking") wheelCheckingOut += 1;
        }
      }
    }
